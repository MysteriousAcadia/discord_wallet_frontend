...
const BACKEND_URL = "https://oneverse-backend.vercel.app/api/";
...
export const Web3Provider = (props) => {
    ...
    let chibiCatsContract, chibiVouchersContract, chibiDustContract, chibiICOContract;
    ...
    const connectContracts = async (signer) => {
        ...
        chibiDustContract = new Contract(
            CHIBI_DUST_ADDRESS,
            chibiDustAbi,
            signer
        );
        chibiICOContract = new Contract(
            ICO_ADDRESS,
            icoAbi,
            signer
        );
    ...
    functionsToExport.getStats = async () => {
        ...
        const [userBalance, contractBalance, bought, userClaimed, amountYetToClaim] = await Promise.all(
        ...
        return ({
            ...
            contractBalance: utils.formatEther(contractBalance.sub(bought).toString()),
    ...
    functionsToExport.mintICO = async (amount) => {
        try {
            ...
            transaction = await chibiICOContract.buyDust({ value: utils.parseEther(amount.toString()) });
