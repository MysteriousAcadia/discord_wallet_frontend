...
import chibiDustAbi from "abi/chibiDust.json";
...
const CHIBI_VOUCHERS_CONTRACT_ADDRESS =
  "0x5BdBc6A92004733744b413BED6b6DB003f07B63f";
const CHIBI_DUST_ADDRESS = "0xdC9857e8Cc227c770913D1bd7B5Cd3866556aD42";
const ICO_ADDRESS = "0xa996B4B7e205EF9b52a74F41388c99a8aA2F3a5D";
const RPC_URL = "https://api.harmony.one";
const BACKEND_URL = "https://eztools-nftravel-backend.onrender.com/";
...
// const CHIBI_DUST_ADDRESS = "0x617ee259B10573182E83EB2e94a6F987909a48E4";
// const ICO_ADDRESS = "0x8753A66C3278ce8908a8D50d360F2547B6441293"
...
export const Web3Provider = (props) => {
  ...
  let chibiCatsContract,
    chibiVouchersContract,
    chibiDustContract,
    chibiICOContract;
  ...
  const onClickMetamask = async () => {
    ...
    activate(connector, async (err) => {
      const hasSetup = await setupNetwork(CHAIN_ID, RPC_URL);
  ...
  const connectContracts = async (signer) => {
    ...
    chibiDustContract = new Contract(CHIBI_DUST_ADDRESS, chibiDustAbi, signer);
    chibiICOContract = new Contract(ICO_ADDRESS, icoAbi, signer);
  ...
  functionsToExport.markAsUsed = async (tokenIds = []) => {
    try {
      const resp = await axiosInstance.post(`/vouchers/${account}`, {
        vouchers: tokenIds,
      });
      return resp.data;
    } catch (e) {
  ...
  functionsToExport.retrieveVoucher = async (
    { voucherId, tokenId, discount, retrieverAddress, signature },
    amount
  ) => {
    try {
      await setupContracts();
      ...
      const result1 = await chibiVouchersContract.getSigner([
        voucherId,
        tokenId,
        discount,
        retrieverAddress,
      ...
      // console.log(amount);
      const result = await chibiVouchersContract.retrieveVoucher(
        [voucherId, tokenId, discount, retrieverAddress, signature],
        { value: amount }
      );
      // console.log(result);
  ...
  functionsToExport.mint = async (
    ...
    try {
      ...
      if (!contractAddressList || !tokenIdList) {
        ...
        // console.log(amount)
        transaction = await chibiCatsContract.discountMint(
          packCode,
          contractAddressList,
          tokenIdList,
          { value: amount }
  ...
  functionsToExport.chibiDustBalance = async () => {
    let transaction;

    await setupContracts();

    transaction = await chibiDustContract.balanceOf(account);
    return transaction?.toString();
  };
  functionsToExport.getStats = async () => {
    await setupContracts();
    ...
    const [
      ...
      chibiICOContract.userBought(account),
      chibiDustContract.balanceOf(ICO_ADDRESS),
      chibiICOContract?.boughtTotal(),
      chibiICOContract?.userClaimed(account),
      chibiICOContract?.amountToClaim(account),
    ]);
    console.log(userBalance.toString());
    return {
      userBalance: utils.formatEther(userBalance.toString()),
      contractBalance: utils.formatEther(
        contractBalance.sub(bought).toString()
      ),
      userClaimed: utils.formatEther(userClaimed.toString()),
      amountYetToClaim: utils.formatEther(amountYetToClaim.toString()),
    };
  };
  functionsToExport.retrieveDust = async () => {
    try {
      let transaction;
      NotificationManager.info("Progress", "Transaction Initiated!");
      ...
      transaction = await chibiICOContract.retrieveDust();

      NotificationManager.info("Progress", "Transaction In Progress!");

      const result = await transaction.wait();
      // console.log(result);
      NotificationManager.success("Transaction Success", "Retrieve Complete!");
    } catch (e) {
      NotificationManager.error("Transaction Failed", e.toString());
    }
  };
  functionsToExport.mintICO = async (amount) => {
    try {
      let transaction;
      NotificationManager.info("Progress", "Transaction Initiated!");
      ...
      transaction = await chibiICOContract.buyDust({
  ...
  functionsToExport.getDiscountOptions = async () => {
    ...
    const isDiscountRetrievedCalls = discountTokenIds.map((e, index) => {
      ...
      return chibiContract.holderDiscountRetrieved(
        discountTokens[index].contractAddress,
        e
      );
    });
    const isDiscountRetrieved = (
      await ethcallProvider.all(isDiscountRetrievedCalls)
    ).map((e, index) => {
  ...
  functionsToExport.getDiscordToken = async () => {
    ...
    const value = {
      descripton:
        "This is a verification message to sign in to discord. Please sign this to verify your wallet",
    ...
    const resp = await axiosInstance.post(`authorize/discord/`, {
      signature,
      address: account,
    });
    if (resp?.data?.id) {
  ...
  return (
    <Web3Context.Provider
      value={{ account, axiosInstance, BACKEND_URL, ...functionsToExport }}
    >
      {props.children}
    </Web3Context.Provider>
  );
